...
namespace test_nodeparser {
    ...
    auto parse(const std::string &str) {
        ...
        auto parser = tconf::toml::Parser{};
        return parser.parse(input);
    }

    TEST_CASE("TestNodeParser, SingleNodeSingleLevel")
    {
        auto result = parse(R"(
    foo = 5
    bar = "test"
    ...
    TEST_CASE("TestNodeParam, DublicateParamError")
    {
        ...
        [a]
          testInt = 10
                ...
                [](const tconf::ConfigError &error) {
                    REQUIRE_EQ(
                            std::string{error.what()},
                            "[line:3, column:15] [error] toml::insert_value: value (\"foo\") already exists.\n"
                            " --> unknown file\n"
                            "   |\n"
                            " 2 |         foo = 5\n"
